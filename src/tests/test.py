...
class LogFileTester:
    ...
    def __init__(self, 
        ...
        if logs_dir is None:
            logs_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', 'login_page', 'src', 'logs')
        if web_log_model_path is None:
            web_log_model_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', 'models', 'web_log_detector_comprehensive.pkl')
        if mouse_movement_model_path is None:
    ...
    def _load_models(self):
        ...
        try:
            if os.path.exists(self.web_log_model_path):
                ...
                print(f"[SUCCESS] Loaded web log model from {self.web_log_model_path}")
            else:
                print(f"[ERROR] Web log model not found at {self.web_log_model_path}")
                return False
                
            if os.path.exists(self.mouse_movement_model_path):
                self.mouse_movement_detector.load_model(self.mouse_movement_model_path)
                print(f"[SUCCESS] Loaded mouse movement model from {self.mouse_movement_model_path}")
            else:
                print(f"[ERROR] Mouse movement model not found at {self.mouse_movement_model_path}")
                return False
            ...
            print("[SUCCESS] Fusion module initialized")
            return True
    ...
    def load_log_file(self, filename: str) -> Dict[str, Any]:
        ...
        try:
            with open(filepath, 'r') as f:
                data = json.load(f)
            print(f"[SUCCESS] Loaded {filename} ({len(data) if isinstance(data, list) else 'object'} entries)")
            return data
        except Exception as e:
    ...
    def preprocess_mouse_movements(self, movements_data: List[str]) -> np.ndarray:
        ...
        try:
            ...
            if len(coordinates) < 2:
                print("[WARNING] Not enough mouse movement coordinates")
                return None
            ...
            print(f"[ERROR] Error preprocessing mouse movements: {e}")
            return None
    ...
    def preprocess_web_logs(self, web_logs: List[Dict]) -> Dict[str, Any]:
        ...
        try:
            ...
            print(f"[ERROR] Error preprocessing web logs: {e}")
            return None
    ...
    def test_mouse_movements_file(self, filename: str = 'mouse_movements.json'):
        ...
        for i, session_data in enumerate(data):
            ...
            if len(movements) < 10:
                print("   [WARNING] Too few movements for reliable detection")
                continue
    ...
    def test_events_file(self, filename: str = 'events.json'):
        ...
        if isinstance(data, dict):
            ...
            if len(mouse_movements) == 0:
                print("   [WARNING] No mouse movements in events file")
                return
            ...
            if matrix is not None:
                try:
                    ...
                    print(f"   [ERROR] Mouse prediction error: {e}")
            ...
            if len(web_logs) > 0:
                ...
                if features is not None:
                    ...
                    try:
                        ...
                        print(f"   [ERROR] Web prediction error: {e}")
